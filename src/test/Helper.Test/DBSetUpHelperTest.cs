...
namespace Codentia.Test.Test.Helper.Test
{
    ...
    public class DBSetUpHelperTest
    {
        ...
        public void _001_EnsureTestDatabaseExists()
        {
            ...
            DBSetupHelper.EnsureTestDatabaseExists("master", "test2", "MITTest2", scriptPath, true);

            // test that it doesnt get rebuilt again
            scriptPath = @"Helper.Test\SQL\SetupTestDB2.sql";
            DBSetupHelper.EnsureTestDatabaseExists("master", "test2", "MITTest2", scriptPath, false);
        }
        ...
        {
            string scriptPath = @"Helper.Test\SQL\SetupTestDB.sql";
            Assert.That(delegate { DBSetupHelper.EnsureTestDatabaseExists("test1", "test1", "MITTest1", scriptPath, true); }, Throws.InstanceOf<Exception>().With.Message.EqualTo("Connection String must be a master database"));
        }
        ...
        public void _003_EnsureTestDatabaseExists_WithError_Coverage()
        {
            ...
            Assert.That(delegate { DBSetupHelper.EnsureTestDatabaseExists("master", "test1", "MITTestBroken", scriptPath, true); }, Throws.InstanceOf<System.Data.SqlClient.SqlException>());
        }
    }
}
